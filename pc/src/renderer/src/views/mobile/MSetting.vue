<template>
  <div class="mainBody">
    <NavBar title="设置">
      <template #left>
        <div>
          <Button plain type="danger" size="small" @click="changeScreen">{{
            !isFullscreen ? '全屏' : '还原'
          }}</Button>
      ...
      <div style="text-align: ;center;margin:30px 30px;display: inline-block;">
        <CheckboxGroup v-model="view.form.VideoTypes" direction="horizontal">
          <Checkbox
            ...
            :key="item"
            :name="item"
            style="margin: 4px 8px; width: 20vw"
          >
            {{ item }}
          </Checkbox>
        </CheckboxGroup>
    ...
    <ActionSheet v-model:show="dirSelect" cancel-text="关闭">
      <div style="text-align: ;center;margin:30px 30px;display: inline-block;">
        <CheckboxGroup v-model="view.form.Dirs" direction="horizontal">
          <Checkbox
            v-for="item in view.form.DirsLib"
            :key="item"
            :name="item"
            style="margin: 4px 8px; max-width: 80vw"
          >
            {{ item }}
          </Checkbox>
        </CheckboxGroup>
    ...
    <ActionSheet v-model:show="tagSelect" cancel-text="关闭">
      <div style="text-align: ;center;margin:30px 30px;display: inline-block;">
        <CheckboxGroup v-model="view.form.Tags" direction="horizontal">
          <Checkbox
            v-for="item in view.form.TagsLib"
            :key="item"
            :name="item"
            style="margin: 4px 8px; max-width: 80vw"
          >
    ...
    <ActionSheet v-model:show="typeSelect" cancel-text="关闭">
      <Search v-model="newType" label="添加" placeholder="请输入添加的类型" @search="addType">
        <template #action>
    ...
    <ActionSheet v-model:show="tagLibSelect" cancel-text="关闭">
      <Search v-model="newTagLib" label="添加" placeholder="请输入添加的标签" @search="addTagLib">
        <template #action>
        ...
        <Tab name="base" title="基础设置">
          ...
          <Row style="margin: 8px auto">
            ...
            <Col :span="18" @click="tagSelect = true">
              <Tag
                v-for="item in view.form.Tags"
                :key="item"
                size="large"
                style="background: green; margin: 2px 4px"
                >{{ item }}</Tag
              >
            </Col>
          </Row>
        ...
        <Tab name="search" title="搜索设置">
          <Row>
            ...
            <Col :span="18" @click="chooseVideoTypes">
              <Tag
                v-for="item in view.form.VideoTypes"
                :key="item"
                size="large"
                style="background: green; margin: 2px 4px"
                >{{ item }}
...
const changeScreen = () => void {
  if (systemProperty.isFullscreene) {
