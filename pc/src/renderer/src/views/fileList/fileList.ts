...
export const formItemTagsChange = (view) => {
  const { Name, Tags, FileType, Code, Actress } = view.formItem
  let newName = ''
  if (Name.indexOf('《') >= 0) {
...
export const formMovieTypeChange = (view: any) => {
  ...
  const { MovieType, Name, FileType } = view.currentFile || {}
  let newName = ''
  if (Name.indexOf('{{') >= 0) {
...
export const fetchTagsLib = (queryString, callback) => {
  ...
  const results = queryString ? suggrestTagsLib.filter(createFilter(queryString)) : suggrestTagsLib
  callback(results)
}
