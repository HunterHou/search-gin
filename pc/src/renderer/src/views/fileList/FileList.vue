<template>
    ...
    <div class="searchRow" :style="searchStyle">
      <ElRow :span="24">
        ...
        <ElCol :xl="2" :lg="2" :md="4" :sm="4" :xs="6">
          <ElRadioGroup v-model="queryParam.SortType" size="default" @change="refreshData">
            <ElRadioButton label="desc">倒</ElRadioButton>
            <ElRadioButton label="asc">正</ElRadioButton>
          </ElRadioGroup>
        </ElCol>
        <ElCol :xl="5" :lg="6" :md="8" :sm="10" :xs="16">
          <ElRadioGroup v-model="queryParam.MovieType" size="default" @change="refreshData">
        ...
        <ElCol :xl="3" :lg="3" :md="5" :sm="5" :xs="8">
          <div style="margin-left: 10px">
            <ElPopover v-model:visible="tagPopover" :width="800" trigger="click">
              <template #reference>
                <ElButton type="success" text link style="margin-left: 10px">标签 </ElButton>
              </template>
            ...
            <ElPopover :width="400" trigger="hover">
              <template #reference>
                <ElButton type="danger" text link bg style="margin-left: 10px">历史 </ElButton>
              </template>
              <template #default>
                  ...
                  <div>
                    ...
                    >
                      <ElButton link bg @click="gotoHistory(item)">
                        【{{ item.SortField }}-{{ item.SortType }}】- {{ item.Page }}-{{
                          item.PageSize
                        }}
                        {{ item.Keyword }}
                        -{{ item.MovieType }}
            ...
            <ElButton
              :type="countTransferIng > 0 ? 'danger' : 'success'"
              text
              link
      ...
      <ElSpace wrap>
        ...
        >
          ...
          >
            <div
              v-if="item"
              :class="isShowCover(view) ? 'img-list-item-cover' : 'img-list-item'"
            ...
            <div
              class="image-tool"
              :style="{
                background: !noMovieType(item.MovieType) ? '' : 'rgb(239 251 219)'
              }"
            >
              <div class="tool-button">
                <ElButton
                  v-if="item.Actress && view.settingInfo.Buttons?.indexOf('图鉴') >= 0"
                  type="warning"
                  plain
                  class="icon-button"
                  title="图鉴"
              ...
              <div class="context-text" style="font-size: 13px">
                ...
                >
                  <template #reference>
                    <span>
                      <ElLink v-if="item.Actress" style="color: green" @click="copy(item.Actress)"
                        >{{ item.Actress }}
                      </ElLink>
                  ...
                  <template #default>
                    ...
                    <ElRow>
                      <span style="font-size: 12px; color: darkred"> {{ item.Title }}</span>
                    </ElRow>
                  </template>
                </ElPopover>
                <span> &nbsp;{{ item.Name }}</span>
  ...
  <ElDialog
    v-model="taskPop"
    :width="windowWidth > 100 ? '80vw' : '500px'"
    :title="`执行任务(${Object.keys(view.transferTask).length - countTransferIng}/${
          ...
          >
            <span
              :style="{
                width: '5rem',
                textAlign: 'left',
                color: item.Status == '成功' ? 'green' : item.Status === '等待' ? 'grey' : 'red'
              }"
            >
              {{ item.Status }}
              {{
                item.Status === '等待'
                  ? ''
                  : (
                      ((new Date(item.FinishTime).getTime() > 0
  ...
  >
    <ElForm label-position="right" :model="view.formItem" size="large" label-width="18%">
      ...
      <ElFormItem label="文件名称">
        <ElInput v-model="view.formItem.Name" type="textarea" autocomplete="off"></ElInput>
      </ElFormItem>
      <ElFormItem label="标签">
        <ElTag
    ...
    <div v-if="view.formItem">
      <div style="margin-top: -40px">
        ...
        <ElRow :gutter="24">
          ...
          <ElCol v-if="view.formItem.Tags" :span="8" tyle="text-align:right">
            <ElTag
              v-for="tag in view.formItem.Tags"
              :key="tag"
              effect="dark"
          ...
          <ElCol :span="8">
            ...
            <span>{{ useDateFormat(view.formItem.MTime, 'YYYY-MM-DD HH:MM:ss') }}</span>
          </ElCol>
        </ElRow>
        <ElRow :gutter="20">
          <ElCol :span="4">
  ...
  <teleport to="body">
    <div v-show="view.innerVisible" class="imageBloswerList" @click="innerVisibleFalse">
      ...
      >
        <ElImage
          style="min-width: 1200px; width: auto; margin: 0 auto; opacity: 9; z-index: 9999"
          :src="item"
        >
  ...
  <teleport to="body">
    <div v-show="view.videoVisible" id="videoDiv" class="playDiv">
      <vue3VideoPlay
      ...
      <div style="background-color: antiquewhite">
        ...
        >
          <el-space spacer="|" wrap>
            ...
            <el-link :underline="false" type="success">{{ view.contextmenuTarget.Code }} </el-link>
            <el-link
              type="warning"
              size="large"
              style="margin-left: 0.5rem"
            ...
            <el-link :underline="false" type="success">{{ view.contextmenuTarget.Name }} </el-link>
          </el-space>
        </div>
        <ElRow v-if="moreTag">
          <el-space spacer="|" wrap>
...
const countTransferIng = computed(() => {
  return Object.keys(view.transferTask)?.filter(
    (item) =>
      view.transferTask[item].Status === '执行中' || view.transferTask[item].Status === '等待'
  ).length
})
...
const handleCheckedCitiesChange = (value) => {
  ...
  view.isIndeterminateDir = checkedCount > 0 && checkedCount < view.settingInfo.Dirs.length
}
...
const loadDirInfo = async (id: string, loading: boolean) => {
  ...
  if (res && res.length > 0) {
    ...
    for (let i = 0; i < res.length; i++) {
      if (res[i].FileType === 'jpg' || res[i].FileType === 'png' || res[i].FileType === 'gif') {
...
const setMovieType = async (id: string, typeId: number) => {
  const movieType = typeId == 4 ? '国产' : typeId == 3 ? '斯巴达' : typeId == 1 ? '步兵' : '骑兵'
