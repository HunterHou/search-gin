<template>
  <div align="center" style="margin-top: -30px">
    <ElForm ref="form" label-width="160px" :model="view.form" label-position="right">
      <h3>
        <ElButton style="float: left" size="default" type="danger" @click="shutDownHandler"
          >关闭计算机
        </ElButton>
        ...
        <ElTabs v-model="activeName" type="card" @tab-click="handleClick">
          <ElTabPane label="扫描设置" name="first">
            ...
            <ElFormItem label="定制按钮">
              <el-checkbox-group v-model="view.form.Buttons" size="large">
                <el-checkbox v-for="item in buttonEnum" :key="item" :label="item">
                  {{ item }}
                </el-checkbox>
              </el-checkbox-group>
            </ElFormItem>
            ...
            <ElFormItem label="已选路徑">
              ...
              <ElCheckboxGroup v-model="view.form.Dirs" @change="handleCheckedCitiesChange">
                <ElCheckbox v-for="dir in view.form.DirsLib" :key="dir" size="default" :label="dir"
                  >{{ dir }}
                </ElCheckbox>
              </ElCheckboxGroup>
            </ElFormItem>
          ...
          <ElTabPane label="系统信息" name="third">
            <div style="text-align: left">
              ...
              <ElInput v-model="view.form.SystemHtml" type="textarea" :rows="8" />
            </div>
          </ElTabPane>
        </ElTabs>
      </div>
      ...
      <div>
        <ElButton class="submit" type="primary" align-text="center" @click="submitForm"
          >提交
        </ElButton>
      </div>
    </ElForm>
...
const makeTabLibData = () => {
  ...
  if (TagsLib) {
    for (let i = 0; i < TagsLib.length; i++) {
