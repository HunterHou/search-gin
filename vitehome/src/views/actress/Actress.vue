...
<template>
  <ElButton class="leftButtom" round @click="pageLoading(-1)">上一页
  </ElButton>
  <ElButton class="rightButtom" round @click="pageLoading(1)">下一页
  </ElButton>
  <ElRow :span="24">
    <ElCol :span="4">
    ...
    <ElCol :span="8">
      <el-input placeholder="请输入内容" v-model="view.Keyword" clearable>
        <template #append>
          <ElButton slot="append" type="primary" size="large" icon="el-icon-search" @click="queryList()">Go!
          </ElButton>
        </template>
      </el-input>
    ...
    <ElCol :span="4" :offset="1">
      <span style="font-size: 22px;margin-top: 20px;"><span>
          扫描：{{ view.TotalCnt }} </span>
      </span>
    </ElCol>
